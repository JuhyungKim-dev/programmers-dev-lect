...
public class task2_0616_Main {

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.println("[1]Lite:10 [2]Basic:20 [3]Premium:30");
        task2_0616_PricePlan plan = null;
        while (plan == null) {
            plan = task2_0616_PricePlan.from(readInt(sc));
            if (plan == null) {
                System.out.println("1~3 중에서 선택하세요.");
            }
        }
        task2_0616_MemberManager manager = new task2_0616_MemberManager(plan.getCapacity());
        while (true) {
            ...
            switch (menu) {
                case 1: {
                    ...
                    task2_0616_Member m;
                    if (grade == 2) {
                        m = new task2_0616_VipMember(name, email, phone);
                    } else {
                        m = new task2_0616_NormalMember(name, email, phone);
                    }
                    manager.add(m);
                ...
                case 2: {
                    System.out.print("이메일 > ");
                    String email = sc.nextLine();
                    task2_0616_Member m = manager.findByEmail(email);
                    if (m == null) {
                ...
                case 3: {
                    System.out.print("이름 > ");
                    String name = sc.nextLine();
                    task2_0616_Member m = manager.findByName(name);
                    if (m == null) {
                        System.out.println("회원이 없습니다.");
                    } else {
                        m.printInfo();
                ...
                case 5: {
                    ...
                    String phone = sc.nextLine();
                    boolean result = manager.update(email, name, newEmail, phone);
                    if (result) {
